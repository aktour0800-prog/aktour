      ...
      <main className="pb-28 pt-24">
        <section className="mx-auto w-full max-w-5xl px-4">
          <div className="overflow-hidden rounded-3xl border bg-white shadow-card">
            <button
              type="button"
              onClick={() => setSelectedImage({ src: "/alaska-mobile/hero-2.webp", alt: "알래스카 여름 대표 이미지" })}
          ...
          <div className="space-y-5">
            {dayPlans.map((day) => (
              <article id={`day-${day.day.replace(/\s+/g, "-").toLowerCase()}`} key={day.day} className="overflow-hidden rounded-2xl border bg-white shadow-card">
                                <button
                  type="button"
                  onClick={() => setSelectedImage({ src: day.image, alt: `${day.day} ${day.route}` })}
                  className="block w-full"
                >
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {day.gallery.map((image, index) => (
                      <button
                        key={`${day.day}-${image}`}
                        type="button"
                        onClick={() => setSelectedImage({ src: image, alt: `${day.day} 갤러리 ${index + 1}` })}
                        className="h-24 w-36 flex-shrink-0 overflow-hidden rounded-lg"
                      >
                        <img
                          src={image}
